...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function setupUploaderAndPreviewSystem() {
      ...
      document.addEventListener('change', async (e) => {
          ...
          // Object Swap Input Visibility Logic
  ...
  async function analyzeReferenceImage(imageFile: File) {
    ...
    try {
      ...
      // Per user request, reset these creative kits to None to allow manual selection
...
4.  Combine these three sources of information into a single, cohesive, and masterful prompt. The prompt should be structured logically (describing the scene, the product, the lighting, the mood, the camera details) and include strong keywords for photorealism and detail. Be creative but grounded in the user's choices.`;
  ...
  function updateWizardControlsState() {
      ...
      const controlsToKeepActive = [
          'reference-usage-select',
          'camera-kit-select',
          'product-retouch-kit-select',
          'manipulation-kit-select',
          'people-retouch-kit-select'
      ];

      allControls.forEach(control => {
          const isControlInActiveList = controlsToKeepActive.some(id => !!control.querySelector(`#${id}`));
          
          // If wizard is on, disable everything EXCEPT the controls in our active list
          const shouldBeDisabled = isChecked && !isControlInActiveList;
  ...
  function buildMasterPrompt(subject: string, negativePrompt: string): string {
    ...
    if (useReference) {
      if (weatherAtmosphere !== 'clear_skies' || seasonOverride !== 'keep_original') {
          ...
          const atmosphereOverride = `CRITICAL OVERRIDE: Regardless of the weather or season shown in the 'Reference Image', the final generated scene MUST have the following atmosphere: **${weatherChange}** and **${seasonChange}**. Apply this new atmosphere to the entire replicated scene. IMPORTANT: Do not obscure the product itself with these effects.`;
...
**FINAL OUTPUT:** A seamless, professional work of art that is a perfect fusion of the product's form and the reference's soul, set within a unique, newly generated environment.`;
          break;
        case 'object_swap':
          const objectToReplace = (document.getElementById('object-replace-input') as HTMLInputElement).value || "the primary object";
          directiveInstruction = `**NON-NEGOTIABLE DIRECTIVE: SURGICAL OBJECT REPLACEMENT.**
**MISSION:** You must replace the object identified as "${objectToReplace}" in the Reference Image with the Subject from the Product Image.

**STEP 1: REMOVAL:** Completely erase "${objectToReplace}" from the scene. Fill the void with the appropriate background/environment details.
**STEP 2: PLACEMENT:** Insert the Subject from the Product Image into the exact position of the removed object.
**STEP 3: ADAPTATION:**
- **Scale & Perspective:** Match the size and angle of the original object.
- **Lighting:** The Subject must be lit EXACTLY by the light sources in the Reference Image.
- **Shadows:** The Subject must cast realistic shadows onto the ground/surfaces, matching the scene's physics.
**STEP 4: FIDELITY:** The Subject MUST remain identical to the Product Image. Do not alter its design, color, or shape.`;
          break;
        case 'lighting_theft':
...
**FINAL OUTPUT:** A single, cohesive, and believable image where the modifications and the integrated product are perfectly blended and appear as if they were captured in a single photograph.`;
        ...
        case 'keep_model_replace_object':
          const modelObjectToReplace = (document.getElementById('object-replace-input') as HTMLInputElement).value || "the object being held";
          directiveInstruction = `**NON-NEGOTIABLE DIRECTIVE: MODEL PRESERVATION & SURGICAL REPLACEMENT.**
**MISSION:** Replace "${modelObjectToReplace}" held/used by the model with the Subject from the Product Image, while keeping the model 100% unchanged.

**RULES:**
1. **PROTECT THE MODEL:** Do NOT change the model's face, hands, body, or clothes. They are locked.
2. **SWAP THE OBJECT:** Remove "${modelObjectToReplace}". Place the Product Image Subject in the model's hands/interaction zone.
3. **REALISTIC GRIP:** Ensure the model's hands appear to be holding the new object naturally.
4. **LIGHTING INTEGRATION:** Relight ONLY the new object to match the scene.`;
          break;
        case 'none':
        default:
          break;
      ...
      directiveInstruction = `CRITICAL COMMAND: This is an inpainting task. You MUST preserve the entire 'Product Image' (subject, objects, composition) with 100% fidelity. Your ONLY task is to apply the following atmospheric changes to the BACKGROUND AND ENVIRONMENT: ${changes}. Do NOT change the product itself or obscure it with weather effects.`;
    } else {
      ...
      switch (weatherAtmosphere) {
          ...
          case 'light_rain': weatherInstruction = "CRITICAL ATMOSPHERE: The scene must have a subtle **light rain or drizzle**. The surrounding environment should appear damp. IMPORTANT: The subject/product must remain clear and visible, NOT obscured by heavy rain streaks."; break;
          case 'heavy_rain': weatherInstruction = "CRITICAL ATMOSPHERE: The scene is in a heavy rain downpour. The background and ground surfaces MUST appear wet with puddles. IMPORTANT: Ensure the product itself remains clearly visible and is not damaged or heavily obscured by the water."; break;
          case 'heavy_fog': weatherInstruction = "CRITICAL ATMOSPHERE: The BACKGROUND must be enveloped in thick, heavy fog or dense mist to create depth. IMPORTANT: The foreground product must remain sharp and clearly visible, popping against the foggy background."; break;
          case 'light_snow': weatherInstruction = "CRITICAL ATMOSPHERE: The scene MUST be rendered during a light snowfall. A gentle dusting of snow should cover the GROUND and BACKGROUND surfaces. IMPORTANT: The product itself must remain clean and free of snow accumulation."; break;
          case 'blizzard': weatherInstruction = "CRITICAL ATMOSPHERE: The BACKGROUND MUST depict a heavy blizzard with blowing snow. IMPORTANT: The product in the foreground must remain the clear focal point and not be completely buried or hidden by the snow."; break;
          ...
          case 'sandstorm': weatherInstruction = "CRITICAL ATMOSPHERE: The scene MUST take place during a sandstorm or in a very dusty environment. The air should be thick with blowing sand or dust particles in the background. IMPORTANT: Ensure the product remains visible and is not lost in the dust."; break;
      ...
      switch (seasonOverride) {
          ...
          case 'winter': seasonInstruction = "CRITICAL SEASON: The entire scene MUST be rendered in Winter. Trees should be bare, and the GROUND may be covered in snow. IMPORTANT: Do not cover the product in snow unless it is logically resting on the ground."; break;
          case 'keep_original': default: break;
      }
      if (seasonInstruction) promptParts.push(`**Season Override:** ${seasonInstruction}`);

      // Subject Protection Clause for Weather/Season
      if (weatherAtmosphere !== 'clear_skies' || seasonOverride !== 'keep_original') {
          const protectionClause = `**CRITICAL DESIGN PROTECTION:**
When applying the requested Weather (${weatherAtmosphere}) or Season (${seasonOverride}), you are **STRICTLY FORBIDDEN** from altering the product's actual design, geometry, or surface details.
1. **NO COVERAGE:** Do NOT cover the product with snow, rain, mud, or leaves. The product must remain clean and fully visible.
2. **NO MODIFICATION:** Do not change the product's color or texture to match the season (e.g., do not make a summer product look "frozen").
3. **ENVIRONMENT ONLY:** Apply the weather effects strictly to the background and the ground AROUND the product.
4. **PRIORITY:** The integrity of the Product Design overrides all atmospheric effects.`;
          promptParts.push(protectionClause);
      }
    }
    ...
    // CRITICAL COMPOSITION RULES (User Request for Strictness)
    const visualWizardCheckbox = document.getElementById('visual-wizard-checkbox') as HTMLInputElement;
    const isVisualWizardActive = visualWizardCheckbox && visualWizardCheckbox.checked;
    
    if (isVisualWizardActive || (weatherAtmosphere && weatherAtmosphere !== 'clear_skies') || (seasonOverride && seasonOverride !== 'keep_original')) {
         promptParts.push("CRITICAL COMPOSITION RULES: 1. Do NOT add any new foreground objects, props, hands, or elements that are not present in the original image. 2. Do NOT change the camera angle or the position of the main subject. 3. Only modify the lighting, color grading, and background atmosphere to match the requested style/season. The main subject must remain exactly as it is, in its original context.");
    }
...
**STEP 3: Apply Style:** Apply this text-described 3D style (including material, lighting, texture, and effects) onto the transparent 3D base model created in Step 1. Render a high-fidelity image that brings this text description to life.`;
        ...
        if (imagePart?.inlineData) {
          ...
          if (selectedColorGrade && selectedColorGrade !== 'none') {
              steps.push(`**Apply Color Grade:** ${get3DColorGradeInstruction(selectedColorGrade)}`);
          }
